...
import { Heart, MessageCircle, Award, Zap, Flame, Users, TrendingUp } from 'lucide-react'
    ...
    <div className="grid grid-cols-1 xl:grid-cols-12 gap-10 animate-in">
      ...
      <div className="xl:col-span-8 space-y-8">
        ...
        <div className="space-y-6">
          {posts.map((post) => (
            <div key={post.id} className="bg-white rounded-[32px] p-6 lg:p-8 shadow-sm border border-gray-100 hover:shadow-md transition-shadow group">
              <div className="flex items-center gap-4 mb-6">
                <div className="w-12 h-12 lg:w-14 lg:h-14 rounded-2xl bg-[#F7F9FC] flex items-center justify-center text-lg lg:text-xl font-bold text-[#1F7A8C] shadow-inner">
                  {post.userName[0]}
                </div>
                <div>
                  <h4 className="font-bold text-base lg:text-lg text-[#0B1320]">{post.userName}</h4>
                  <p className="text-[10px] lg:text-xs text-gray-400 font-bold uppercase tracking-wider">{post.timestamp}</p>
                </div>
                <div className="ml-auto">
                  <div className="bg-[#F4C430]/10 p-2 lg:p-2.5 rounded-xl text-[#F4C430] group-hover:scale-110 transition-transform">
                    <Award size={20} className="lg:w-6 lg:h-6" />
                  </div>
                </div>
              </div>

              <div className="bg-[#F7F9FC] p-5 lg:p-6 rounded-2xl mb-6 border border-gray-50">
                <p className="text-[#0B1320] text-base lg:text-lg leading-relaxed font-medium">
                  <span className="text-[#1F7A8C] font-black">{post.userName}</span> {post.action}
                </p>
              </div>
              ...
              <div className="flex items-center justify-between pt-4 border-t border-gray-50">
                <div className="flex items-center gap-4 lg:gap-6">
                  ...
                  <button className="flex items-center gap-2 text-xs lg:text-sm font-bold text-gray-400 hover:text-[#1F7A8C] transition-colors">
                    ...
                    8 <span className="hidden sm:inline">Comentários</span>
                  </button>
                </div>
      ...
      <div className="xl:col-span-4 space-y-8">
        ...
        <div className="bg-white rounded-[40px] p-8 shadow-sm border border-gray-100 hidden xl:block">
          ...
          <div className="space-y-4">
            {[
              { label: '#ReservaDeEmergência', users: '2.4k' },
              { label: '#InvestindoDoZero', users: '1.8k' },
              { label: '#MinhaPrimeiraAção', users: '950' },
          ...
          <div className="space-y-5">
            {[
              ...
              <div key={i} className="flex items-center gap-4">
                <div className="w-12 h-12 rounded-xl bg-[#F7F9FC] flex items-center justify-center text-lg">{user.icon}</div>
